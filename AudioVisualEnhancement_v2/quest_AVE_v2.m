...
% because im updating only in the trials with stimulus, i chabge the
% threshold to 0.5 *because now the baseline is not 0.5. If you press 
pThreshold=0.5; % (previous was 0.72 and gamma 0.5) 
beta=3.5;delta=0.01;
gamma=0.0; % gamma is the proportion correct of responses that are expected when stimulus is not present. slope, threshold and minimum value (it is a 3AFC, noise, 2 x tilt )
...
% el 50% de las veces
see_quest = 0;
range = 0.2; % range below and above threshold to be tested
grain = 0.01;
    ...
    switch state
        ...
        case 'Initialize'
            ...
            if (iTrial  >= quest.nTrials)
                ...
                if(strcmp(quest.stim_conditions(target_definition),'signal'))
                ...
                noisetex = Screen('MakeTexture', win,  img); %Texture generated
